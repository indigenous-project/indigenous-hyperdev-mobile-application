...
//User fill the topic title, description , add an image, add a category for a new discussion.
//The new discussion must be approved by admin before publishing.
function CreateDiscussion(props) {
  ...
  const handlePostDiscussion = () => {
    // prevent empty input
    if (!discussionTitle) {
      Alert.alert('Required!', 'To Proceed, you need to write Dicussion Topic.');
      return;
    }
    if (!discussionDescription) {
      Alert.alert('Required!', 'To Proceed, you need to write Description.');
      return;
    }
  ...
  return (
    <View style={styles.baseModal}>
      ...
      <MessageModal
        showing={showing}
        message={`\tDiscussion Created.\n\nYour Discussion will be published after it is approved.`}
      />
      <ScrollView>
        <View>
